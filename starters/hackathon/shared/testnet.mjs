...
export async function waitForTestnetLedgerTime(unixSeconds, {
  attempts = 90,
  horizon = STELLAR_TESTNET.horizon,
} = {}) {
  if (!Number.isSafeInteger(unixSeconds) || unixSeconds <= 0) {
    throw new Error("target ledger time must be a positive whole Unix timestamp");
  }
  if (!Number.isSafeInteger(attempts) || attempts < 1 || attempts > 180) {
    throw new Error("ledger-time attempts must be an integer from 1 through 180");
  }
  const endpoint = new URL("/ledgers", horizon);
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    if (response?.ok) {
      ...
      const parsed = typeof closedAt === "string" ? Date.parse(closedAt) : Number.NaN;
      if (Number.isFinite(parsed)) {
        lastLedgerTime = Math.floor(parsed / 1_000);
        if (lastLedgerTime >= unixSeconds) return lastLedgerTime;
      }
    } else if (
      response
      && response.status !== 429
      ...
      throw new Error(`Horizon could not read the latest testnet ledger (HTTP ${response.status})`);
    }
    await sleep(2_000);
  }
  throw new Error(
    `testnet ledger time did not reach ${unixSeconds}; latest observed ${lastLedgerTime ?? "unknown"}`,
  );
}
