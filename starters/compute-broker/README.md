...
This self-contained REAPP starter protects `GET /compute/sha256-chain/:tier/:seedId` with a request-bound v2 payment on Stellar testnet. The MandateRegistry contract is the spending authority; the SDK and this application are untrusted clients of that contract.
...
- Negative path: `compute-tier-over-ceiling` — The deterministic hash function rejects an iteration count above 512 before hashing begins.
