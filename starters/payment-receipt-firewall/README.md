...
This self-contained REAPP starter protects `GET /vault/:resourceId` with a request-bound v2 payment on Stellar testnet. The MandateRegistry contract is the spending authority; the SDK and this application are untrusted clients of that contract.
...
- Negative path: `durable-replay-after-restart` — After restart the exact same proof returns the byte-identical stored response without repeating verification or paid fulfillment.
