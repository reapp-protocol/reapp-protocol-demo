...
export default function Docs() {
  return (
    <main className="relative mx-auto w-full max-w-3xl px-4 py-12 sm:px-5">
      ...
      <motion.div {...fade()}>
        <div className="text-[11px] font-semibold tracking-widest text-emerald-400/80">@reapp-sdk/core · DOCS</div>
        <h1 className="mt-3 text-3xl font-bold tracking-tight sm:text-5xl">
          Agent payments in{" "}
          <span className="bg-gradient-to-r from-emerald-300 to-teal-300 bg-clip-text text-transparent">5 lines</span>.
        </h1>
...
function Code({ children }: { children: string }) {
  return (
    <pre className="overflow-x-auto rounded-xl border border-white/10 bg-black/40 p-4 text-xs leading-relaxed text-emerald-100/90">
      <code>{children}</code>
    </pre>
  );
}
