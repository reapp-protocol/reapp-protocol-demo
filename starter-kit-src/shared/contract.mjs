...
export async function expectVerifiedBudgetRejection({
  ...
  try {
    ...
    throw new DeliveryPendingError(
      receipt,
      new Error("payment unexpectedly settled when the contract budget should reject it"),
    );
  } catch (error) {
    if (isBudgetRejection(error, mandate.id)) {
      return Object.freeze({
...
export function isBudgetRejection(error, mandateId) {
  ...
  return /Error\(Contract,\s*#6\)/.test(cause);
}
