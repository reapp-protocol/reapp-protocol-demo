...
The page generates one setup command for the starter you choose. You only copy and run it; the command downloads the ZIP, verifies its exact published SHA-256, extracts it into the empty folder, removes the ZIP, and installs the locked dependencies. A changed or incomplete archive is deleted before extraction.
...
Every starter folder is self-contained. Replace `hackathon` with any of the other starter slugs in the table below.
